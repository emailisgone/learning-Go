...
func (paddle *paddle) draw(pixels []byte) {
	...
	startY := int(paddle.y - paddle.w/2)
...
func main() {
	...
	window, err := sdl.CreateWindow("Amongus Pong-us 🤫(SUS ver.)", sdl.WINDOWPOS_UNDEFINED, sdl.WINDOWPOS_UNDEFINED, int32(winWidth),
	...
	for {
		...
		if state == play {
			if keyState[sdl.SCANCODE_RCTRL] != 0 {
				state = pause
			}
		} else if state == pause {
			if keyState[sdl.SCANCODE_RCTRL] != 0 {
